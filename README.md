...
1. View
   - Input
     ...
     - [ ] 사용자로부터 이동 횟수(라운드 수) 를 입력받을 수 있다.
   - Output
...
2. Model
   - Car
     - [ ] 무작위 값(0~9) 중 4 이상이 나오면 전진할 수 있다.
     - [ ] 현재 이동 거리를 반환할 수 있다.
   - Winner
     - [ ] 모든 자동차의 이동 거리 정보를 기반으로 우승자(들) 을 결정할 수 있다.
...
5. Validator
   - [ ] 자동차 이름 입력값의 형식을 검증할 수 있다. (특수문자는 쉼표(,)만 허용)
   - [ ] 자동차 이름 유효성을 검증할 수 있다. (중복 불가, 특수문자 불가, 5자 이하)
   - [ ] 이동 횟수 입력값을 검증할 수 있다. (1 이상 10 이하의 숫자만 허용)
